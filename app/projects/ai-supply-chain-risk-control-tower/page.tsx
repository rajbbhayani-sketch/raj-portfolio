...
const corridors = {
  "Rotterdam - Ruhr - Central Europe": {
    context:
      "High-volume western corridor with port dependency, inland transport pressure, and industrial supplier exposure.",
    baseRisk: 18,
  },
  "Hamburg - Berlin - Poland": {
    context:
      "Northern and eastern corridor with road, rail, and cross-border reliability considerations.",
    baseRisk: 15,
  },
  "Antwerp - Benelux - France": {
    context:
      "Dense logistics region where port congestion and supplier lead-time variance can affect retail and manufacturing flows.",
    baseRisk: 16,
  },
};
...
export default function AiSupplyChainRiskControlTower() {
  const [corridor, setCorridor] = useState<keyof typeof corridors>(
    "Rotterdam - Ruhr - Central Europe",
  );
  ...
  const result = useMemo(() => {
    ...
    const delayProbability = Math.min(
      ...
      Math.round(
        supplierDelay * 0.38 +
          portCongestion * 0.32 +
          (100 - supplierReliability) * 0.18 +
          8,
      ),
    );
    const shortageRisk = Math.min(
      99,
      Math.round(
        inventoryRisk * 0.52 + demandVolatility * 0.28 + supplierDelay * 0.16,
      ),
    );
    const resilience = Math.max(
      4,
      100 - Math.round(riskScore * 0.72 + shortageRisk * 0.18),
    );
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 py-14 lg:grid-cols-[0.58fr_0.42fr] lg:items-center">
        <div>
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-600">
            ...
            European logistics corridors and turns supplier, port, demand, and
            inventory signals into practical mitigation actions.
          </p>
        ...
        <Image
          src="/projects/ai-supply-chain-demo-screenshot.png"
          alt="Supply chain risk and disruption control tower screenshot"
          width={1366}
          height={700}
          priority
      ...
      <section className="border-y border-slate-200 bg-[#f5f5f7] px-5 py-10">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-3">
          <InfoCard
            icon={Route}
            title="Corridor Risk"
            text="Scores supplier delay, port congestion, inventory cover, demand volatility, and supplier reliability."
          />
          <InfoCard
            icon={PackageCheck}
            title="Inventory Exposure"
            text="Highlights shortage pressure and helps teams decide when to increase safety stock or expedite."
          />
          <InfoCard
            icon={ShipWheel}
            title="German & European Logistics"
            text="Frames risk around ports, cross-border lanes, industrial suppliers, and transport bottlenecks."
          />
        </div>
      </section>
      ...
      >
        <aside className="rounded-lg border border-slate-200 bg-white p-6">
          ...
          <select
            ...
            value={corridor}
            onChange={(event) =>
              setCorridor(event.target.value as keyof typeof corridors)
            }
          ...
          <Slider
            label="Supplier delay signal"
            value={supplierDelay}
            setValue={setSupplierDelay}
            suffix="%"
            min={0}
            max={100}
          />
          ...
          <Slider
            ...
            setValue={setSupplierReliability}
            suffix="%"
            min={35}
            max={100}
          />
        ...
        <div className="grid gap-5">
          ...
          <OutputPanel
            title="Recommended Supply Chain Actions"
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <p className="mt-2 leading-7 text-slate-600">
              This live demo uses transparent scoring and scenario logic in the
              browser. It demonstrates how I would structure a practical
              risk-control dashboard before connecting it to ERP, WMS,
              transport, supplier, port, and forecast data.
            </p>
          </div>
        </div>
      </section>
